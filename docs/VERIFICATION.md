...
| 18 | No major streaming stalls | Worker priority/cancellation tests and 4.87 ms CPU P95 | Awaiting live 1080p frame profile |
...
Open `http://localhost:5190`, hard-refresh, and use `window.__BARSOOM__` to set the required altitudes and locations. Capture the canvas after tile loading settles and record telemetry from `window.__BARSOOM__.getState().telemetry`.
...
|---|---|---|
| 30,000 km | Complete planet, surrounding sky, stars/planet, thin limb | Pending screenshot + telemetry |
| 10,000 km | Terminator readability and global colour coherence | Pending screenshot + telemetry |
| 1,000 km | Regional MOLA relief and clean LOD transition | Pending screenshot + telemetry |
| 100 km | Horizon haze, no cracks or black tiles | Pending screenshot + telemetry |
| 10 km | Local terrain density and material continuity | Pending screenshot + telemetry |
| 1 km | Stable depth, no swimming or repeated texture pattern | Pending screenshot + telemetry |
| 100 m | Metre detail, pan precision and terrain collision | Pending screenshot + telemetry |
| 0 m AGL | Surface epsilon, horizon atmosphere and no clipping | Pending screenshot + telemetry |

...
Each of the six root faces persists in a quadtree. Selection uses projected geometric error, viewport height, FOV, camera distance, a frustum sphere test, and a planetary horizon-angle test. Coarse parents are requested first. Children become visible only after all four are ready; a dithered fade and parent-height morph then replaces the parent without transparent overlap lines during ordinary survey navigation. Above the orbital-imagery handoff altitude, a depth-tested coverage sphere sits below the lowest Martian relief and renders after terrain. It is invisible under healthy coverage, while a transient streaming void reveals a stable, correctly registered and sunlight-matched copy of the Mars orbital albedo instead of background black or an animated effect. Target-locked moons and spacecraft can translate hundreds of kilometres per real second at the selectable accelerated rates up to 60×, so that view holds terrain at the native MOLA asset ceiling and swaps a complete child quartet atomically rather than displaying a moving dither transition.
...
Phobos, Deimos, Odyssey, MRO, and TGO each project a moving, screen-readable DOM acquisition marker from their physical Mars-fixed position. The marker is omitted when Mars occults the target and is clickable to enter the same target-lock camera that remains available through the identity menu. Spacecraft paths use NASA/JPL Horizons osculating elements and render as low-cost orbit lines in survey mode. Detailed official spacecraft models are lazy-loaded only for the selected target, scaled to their published deployed dimensions, kept one-at-a-time, and disposed when the lock changes. This keeps discovery legible without enlarging the simulated object or adding model cost to the globe view.

...
The default wheel descent remains nadir at planetary scale and eases to a 48-degree-from-vertical RTS composition between 350 km and 25 km AGL. The pitch changes camera offset while re-solving distance against terrain, so it does not alter requested altitude. A middle-mouse orbit immediately disables the assist and leaves the unrestricted quaternion orientation authoritative; right-mouse pan continues to translate camera and focus in the existing view plane.

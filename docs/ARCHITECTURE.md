...
Each of the six root faces persists in a quadtree. Selection uses projected geometric error, viewport height, FOV, camera distance, a frustum sphere test, and a planetary horizon-angle test. Coarse parents are requested first. Children become visible only after all four are ready; a dithered fade and parent-height morph then replaces the parent without transparent overlap lines. Above the orbital-imagery handoff altitude, a depth-tested coherence substrate sits below the lowest Martian relief and renders after terrain. It is therefore invisible under healthy coverage, while a transient streaming void reveals a violet/cyan interference field and displaced albedo echo instead of background-black; the advancing LOD threshold carries a faint matching spectral edge.

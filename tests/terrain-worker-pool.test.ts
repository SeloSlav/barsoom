...
describe("terrain worker scheduling", () => {
  it("removes a rapid burst of obsolete queued jobs instead of letting the backlog grow", async () => {
    ...
    const active = [
      pool.request({ face: "px", lod: 1, x: 0, y: 0 }, base, 10),
      pool.request({ face: "px", lod: 1, x: 1, y: 0 }, base, 9),
    ];
  ...
  it("dispatches the highest-screen-importance queued tile first", async () => {
    const pool = new TerrainWorkerPool();
    const first = pool.request({ face: "px", lod: 1, x: 0, y: 0 }, base, 1);
    const second = pool.request({ face: "px", lod: 1, x: 1, y: 0 }, base, 1);
    ...
    const promises = [first, second, low, high, medium].map((job) => job.promise.catch(() => null));
    const worker = FakeWorker.instances[0];
    worker.complete(first.id);
